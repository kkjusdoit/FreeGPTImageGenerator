...
class ImportAuthorizedAccountReq(BaseModel): raw_text: str
...
class ImportSessionJsonReq(BaseModel): raw_text: str
...
async def get_accounts(
    ...
    hide_reg: str = Query("0"),
    account_filter: str = Query("all"),
    token: str = Depends(verify_token),
):
    result = db_manager.get_accounts_page(page, page_size, hide_reg=hide_reg, account_filter=account_filter)
...
@router.post("/api/accounts/import_authorized")
async def import_authorized_accounts(req: ImportAuthorizedAccountReq, token: str = Depends(verify_token)):
    if not req.raw_text:
    ...
    if not parsed_accounts:
        return {"status": "error", "message": "未识别到有效授权账号，请使用 email----password----client_id----refresh_token 格式"}

    inserted, updated = db_manager.import_authorized_accounts(parsed_accounts)
    return {
        "status": "success",
        "message": f"已导入授权账号：新增 {inserted} 个，更新 {updated} 个。未执行联网验证。",
        "inserted": inserted,
        "updated": updated,
    }
...
def account_action(data: dict, token: str = Depends(verify_token)):
    try:
        ...
        token_meta = db_manager.classify_token_payload(token_data)

        if action in {"push", "push_sub2api"} and not token_meta.get("cpa_eligible"):
            return {"status": "error", "message": f"账号 {email} 仅注册成功或凭证不完整，已排除出后续 CPA 流程。"}
...
@router.post("/api/accounts/import_session_json")
async def import_session_json(req: ImportSessionJsonReq, token: str = Depends(verify_token)):
    """接受从 https://chatgpt.com/api/auth/session 复制的 JSON（支持批量粘贴），自动提取 accessToken + email 入库。"""
    import json as _json, base64 as _base64

    if not req.raw_text or not req.raw_text.strip():
        return {"status": "error", "message": "内容为空，请粘贴完整的 Session JSON"}
    ...
    for line in raw.splitlines():
        ...
        if line.startswith("{") and line.endswith("}"):
            try:
                json_objects.append(_json.loads(line))
            except Exception:
                pass
    ...
    if not json_objects:
        try:
            ...
            if isinstance(obj, dict):
                ...
                json_objects = [o for o in obj if isinstance(o, dict)]
        except Exception:
            pass
        ...
        for i, ch in enumerate(raw):
            if ch == "{":
                ...
                if depth == 0 and start is not None:
                    try:
                        json_objects.append(_json.loads(raw[start:i+1]))
                    except Exception:
                        pass
                    start = None

    if not json_objects:
        return {"status": "error", "message": "无法解析 JSON，请确保粘贴了完整的 Session JSON"}
    ...
    for obj in json_objects:
        ...
        access_token = str(obj.get("accessToken", "")).strip()
        user_info = obj.get("user") or {}
        email = str(user_info.get("email", "")).strip().lower()
        name = str(user_info.get("name", "")).strip()
        picture = str(user_info.get("image", "") or user_info.get("picture", "")).strip()

        if not access_token:
            errors.append("某条记录缺少 accessToken，已跳过")
            continue

        # 若没有 email，从 JWT payload 提取
        if not email:
            try:
                ...
                if len(parts) >= 2:
                    ...
                    claims = _json.loads(_base64.urlsafe_b64decode(payload))
                    email = str(claims.get("email", "")).strip().lower()
            except Exception:
                pass

        if not email:
            errors.append("某条记录无法提取 email，已跳过")
            continue
        ...
        token_json = _json.dumps({
            ...
            "source": "chatgpt_session_api",
        }, ensure_ascii=False)

        try:
            with db_manager.get_db_conn() as conn:
                c = db_manager.get_cursor(conn)
                db_manager.execute_sql(c, "SELECT id, password FROM accounts WHERE LOWER(TRIM(email)) = LOWER(TRIM(?))", (email,))
                row = c.fetchone()
                if row:
                    orig_pwd = row[1] if row[1] else ""
                    db_manager.execute_sql(c, "UPDATE accounts SET token_data = ? WHERE email = ?", (token_json, email))
                    updated += 1
                else:
                    db_manager.execute_sql(c, "INSERT INTO accounts (email, password, token_data) VALUES (?, ?, ?)", (email, "", token_json))
                    inserted += 1
            
            # 对接 chatgpt2api 项目
            try:
                db_manager.sync_token_to_chatgpt2api(email, access_token)
            except Exception:
                pass
        except Exception as e:
            errors.append(f"{email}: 写库失败 ({e})")

    total = inserted + updated
    if total == 0:
        return {"status": "error", "message": f"没有有效记录导入。{'; '.join(errors) if errors else '请检查 JSON 内容'}"}

    msg = f"Session JSON 导入完成：新增 {inserted} 个，更新 {updated} 个。"
    if errors:
        msg += f" 部分跳过：{'；'.join(errors)}"
    return {"status": "success", "message": msg, "inserted": inserted, "updated": updated}
